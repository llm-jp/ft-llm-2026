...
# Copyright (c) 2025, LLM-jp.  All rights reserved.
#
...
def convert_layer(model, layer: int, torch_dtype) -> dict:
    ...
    q_weights_base_name: str = f"model.layers.{layer}.self_attn.q_proj.weight"
    k_weights_base_name: str = f"model.layers.{layer}.self_attn.k_proj.weight"
    v_weights_base_name: str = f"model.layers.{layer}.self_attn.v_proj.weight"
    ...
    o_weight_base_name = f"model.layers.{layer}.self_attn.o_proj.weight"
    converted[o_weight_base_name] = param_to_weights(o_weight)

    # mlp
    mlp_weights = model.state_dict()[
        f"model.decoder.layers.{layer}.mlp.linear_fc1.weight"
    ]
    mlp_down_proj_weight = mlp_weights[:ffn_hidden_size, :]
    mlp_gate_proj_weight = mlp_weights[ffn_hidden_size:, :]

    mlp_down_proj_base_name = f"model.layers.{layer}.mlp.gate_proj.weight"
    mlp_gate_proj_base_name = f"model.layers.{layer}.mlp.up_proj.weight"
    ...
    mlp_up_proj_weight = model.state_dict()[
        f"model.decoder.layers.{layer}.mlp.linear_fc2.weight"
    ]
    mlp_up_proj_base_name = f"model.layers.{layer}.mlp.down_proj.weight"
    ...
    input_ln_base_name = f"model.layers.{layer}.input_layernorm.weight"
...
def convert(
    ...
    checkpoint["model.norm.weight"] = param_to_weights(final_ln_weight)
    ...
    if model_config.get("megatron_amp_O2", False):
        ...
        for key in keys:
            checkpoint[key.replace("model.", "model.", 1)] = checkpoint.pop(key)
...
def get_input_hf_path(config_path: str) -> str:
    with open(config_path) as file:
        config = yaml.safe_load(file)
    return config["tokenizer"]["type"]
...
def main():
    ...
    torch_dtype = convert(
        ...
        output_hf_file=f"{args.output_hf_path}/pytorch_model.bin",
        precision=args.precision,
        cpu_only=args.cpu_only,
        n_jobs=args.n_jobs,
    )
    replace_hf_weights_and_tokenizer(
        weights_file=f"{args.output_hf_path}/pytorch_model.bin",
        torch_dtype=torch_dtype,
        input_hf_path=get_input_hf_path(f"{args.input_name_or_path}/model_config.yaml"),
        chat_template=args.chat_template,
        output_hf_path=args.output_hf_path,
    )
    os.remove(f"{args.output_hf_path}/pytorch_model.bin")
