...
from sft_dataset import SFTModelDataset, create_sft_dataloader, load_sft_datasets
...
def _modify_config(gpt_cfg, cfg, add_cfg_to_tree=False):
    ...
    with open_dict(gpt_cfg):
        ...
        if cfg.model.get("expert_model_parallel_size", 1) > 0:
            gpt_cfg.expert_model_parallel_size = cfg.model.get(
...
def main(cfg):
    ...
    train_examples, dev_examples = load_sft_datasets(cfg, ptl_model.tokenizer)

    init_distributed(trainer, ptl_model, cfg.model.get("transformer_engine", False))

    train_dataset = SFTModelDataset(
        tokenized_examples=train_examples,
        tokenizer=ptl_model.tokenizer,
        max_seq_length=cfg.model.max_seq_length,
    )
    val_dataset = SFTModelDataset(
