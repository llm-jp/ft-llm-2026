...
ALL_PARAMS+=(
    ...
    --tokenizer-model ${ENV_DIR}/src/llm-jp-tokenizer/models/ver4.0_alpha1.0/llm-jp-tokenizer_ver4.0_alpha1.0.model
)
...
# Overrides for scheduler decay: 継続事前学習の設定はこちらに書く
# math.ceil(1_000_000_000 / 512 / 4096) = 477 <- global batch size 512，context length 4096 で1B tokens を学習するのに必要なステップ数
# iteration が 100-10000 までの間はチェックポイントの保存頻度が 100 なので 477 より大きい 500 に設定
# チェックポイント保存のアルゴリズムは以下を参照．
# https://github.com/llm-jp/Megatron-LM/blob/3ff6479d0fc193ade5c313956a3f5ccc5682c1e7/megatron/training/training.py#L623
ALL_PARAMS+=(
    --min-lr 0
    --lr-warmup-iters 50
    --train-iters 500
